...
val rootDir = s"/user/byr23/tpcds/sf$scaleFactor-$format/useDecimal=$useDecimal,useDate=$useDate,filterNull=$filterNull"
// name of database to be created.
val databaseName = s"""tpcds_sf${scaleFactor}_${if (useDecimal) "with" else "no"}decimal_${if (useDate) "with" else "no"}date_${if (filterNull) "no" else "with"}nulls"""
// Create the table schema with the specified parameters.
import com.databricks.spark.sql.perf.tpcds.TPCDSTables
val tables = new TPCDSTables(spark.sqlContext, dsdgenDir = "/home/byr23/tpcds-kit/tools", scaleFactor = scaleFactor, useDoubleForDecimal = !useDecimal, useStringForDate = !useDate)
